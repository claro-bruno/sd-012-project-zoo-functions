...
function getOldestFromFirstSpecies(id) {
  // seu código aqui
  const fistAnimalId = employees.find((employee) => employee.id === id).responsibleFor[0];
  const { residents } = species.find((animal) => fistAnimalId === animal.id);
  const oldSpecie = Object.values(residents.sort((animalA, animalB) => animalB.age - animalA.age)[0]);
  return oldSpecie;
}
console.log(getOldestFromFirstSpecies('4e086c7819a2'));