...
function getAnimalsOlderThan(animal, age) {
  const sp = data.species.filter((specie) => specie.name === animal);
  const older = sp[0].residents.every((resident) => resident.age >= age);
  return older;
}
...
function getSchedule(dayName) {
  const serio = data.hours.Wednesday.open;
  const schedule = {
    Tuesday: `Open from ${data.hours.Tuesday.open}am until ${data.hours.Tuesday.close - 12}pm`,
    Wednesday: `Open from ${serio}am until ${data.hours.Wednesday.close - 12}pm`,
...
function getOldestFromFirstSpecies(id) {
  ...
  const filterId = data.species.filter((specie) => specie.id === findEmployee.responsibleFor[0]);
